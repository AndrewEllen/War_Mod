...
- **Automation:** either workbench half accepts compatible bodies, chips and warheads/controllers from any side, including unsided inventory access, into their keyed input slots. Either half exposes only the virtual completed missile for extraction; components cannot be extracted through automation. Hoppers and item pipes use the same transactional rule as the output slot.
- **Tabletop preview:** the body and warhead/controller are shown in the assembly cradle as soon as they are inserted. Targeting chips stay inside the keyed electronics tray.
