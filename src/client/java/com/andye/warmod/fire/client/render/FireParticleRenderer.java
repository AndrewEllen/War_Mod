...
public final class FireParticleRenderer {
    ...
    public static void renderFlames(final PoseStack.Pose pose, final VertexConsumer buffer,
        ...
        renderFlames(pose, buffer, gameTime, cells, camera, false);
    }

    public static void renderFlames(final PoseStack.Pose pose, final VertexConsumer buffer,
        final double gameTime, final List<FireRenderCell> cells,
        final Quaternionf camera, final boolean densePass) {
        long started = System.nanoTime();
        Basis basis = Basis.from(camera, pose);
        for (FireRenderCell render : cells) {
            float passWeight = densePass ? render.closeDetail() : 1.0F - render.closeDetail();
            if (passWeight <= 0.001F) continue;
            ...
            for (int index = 0; index < render.plan().flames().size(); index++) {
                ...
                Vec3 base = render.cell().band().exactPatch()
                    ? render.cell().centroid().add(card.position()
                        .subtract(render.cell().centroid()).scale(1.0 - progress * 0.48))
                    : card.position();
        ...
        for (FireRenderCell render : cells) {
            ...
            for (int index = 0; index < samples; index++) {
                ...
                Vec3 center = source.position().add(
                    ...
                    .subtract(render.cameraPosition());
    ...
    private static void billboard(final PoseStack.Pose pose, final VertexConsumer buffer,
        final Vec3 center, final float radius, final float rotation,
        final int red, final int green, final int blue, final float alpha,
        final int light, final Basis basis, final float aspect) {
        float cos = Mth.cos(rotation), sin = Mth.sin(rotation);
        float ux = cos * radius, uy = sin * radius * aspect;
        float vx = -sin * radius, vy = cos * radius * aspect;
        // One transformed centre per card, rather than a matrix transform and
        // temporary Vector3f for each of its four vertices. The shared billboard
        // axes and normal were already transformed once for the entire pass.
        Vector3f transformedCenter = pose.pose().transformPosition(
            (float)center.x, (float)center.y, (float)center.z, new Vector3f());
        int alphaByte = Mth.clamp((int) (alpha * 255.0F), 0, 255);
        vertex(pose, buffer, transformedCenter, -ux - vx, -uy - vy, 0.0F, 1.0F,
            red, green, blue, alphaByte, light, basis);
        vertex(pose, buffer, transformedCenter, -ux + vx, -uy + vy, 0.0F, 0.0F,
            red, green, blue, alphaByte, light, basis);
        vertex(pose, buffer, transformedCenter, ux + vx, uy + vy, 1.0F, 0.0F,
            red, green, blue, alphaByte, light, basis);
        vertex(pose, buffer, transformedCenter, ux - vx, uy - vy, 1.0F, 1.0F,
            red, green, blue, alphaByte, light, basis);
    }

    private static void vertex(final PoseStack.Pose pose, final VertexConsumer buffer,
        final Vector3f center, final float x, final float y, final float u, final float v,
        ...
        // BufferBuilder has a contiguous entity-format fast path for this
        // overload. Separate setters repeat element lookup/bookkeeping six
        // times per vertex, which dominates dense-fire CPU profiles.
        int color = alpha << 24 | red << 16 | green << 8 | blue;
        buffer.addVertex(center.x + ox, center.y + oy, center.z + oz,
            color, u, v, 0, light, basis.normalX, basis.normalY, basis.normalZ);
    }
    ...
    private static final class Basis {
        ...
        private static Basis from(final Quaternionf camera, final PoseStack.Pose pose) {
            Quaternionf orientation = camera == null ? new Quaternionf() : camera;
            return new Basis(pose.pose().transformDirection(new Vector3f(1, 0, 0).rotate(orientation)),
                pose.pose().transformDirection(new Vector3f(0, 1, 0).rotate(orientation)),
                pose.transformNormal(new Vector3f(0, 0, 1).rotate(orientation), new Vector3f()));
        }
    }
}
