...
public final class IcbmMissileMesh {
	...
	public static void render(final PoseStack.Pose pose, final VertexConsumer buffer,
		final IcbmLongRangeRenderContext.Lod lod, final int light) { render(pose, buffer, lod, light, 255); }
	public static void render(final PoseStack.Pose pose, final VertexConsumer buffer,
		final IcbmLongRangeRenderContext.Lod lod, final int light, final int alpha) {
		int sides = lod == IcbmLongRangeRenderContext.Lod.NEAR ? 12
			: lod == IcbmLongRangeRenderContext.Lod.MEDIUM ? 8 : 6;
	...
	private static void quad(final PoseStack.Pose p,final VertexConsumer b,
		...
		final int r,final int g,final int bl,final int a,final int light,final float nx,final float ny,final float nz){
		vertex(p,b,x1,y1,z1,0,1,r,g,bl,a,light,nx,ny,nz);vertex(p,b,x2,y2,z2,0,0,r,g,bl,a,light,nx,ny,nz);
		vertex(p,b,x3,y3,z3,1,0,r,g,bl,a,light,nx,ny,nz);vertex(p,b,x4,y4,z4,1,1,r,g,bl,a,light,nx,ny,nz);
	}
	private static void vertex(final PoseStack.Pose p,final VertexConsumer b,final float x,final float y,final float z,
		final float u,final float v,final int r,final int g,final int bl,final int a,final int light,
		final float nx,final float ny,final float nz){b.addVertex(p,x,y,z).setColor(r,g,bl,a).setUv(u,v)
