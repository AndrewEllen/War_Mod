...
public record IcbmVisualState(IcbmFlightPlan flightPlan) {
	...
	public IcbmVisualState withGuidance(final ClientboundIcbmGuidanceUpdatePayload p){IcbmFlightPlan old=flightPlan;return new IcbmVisualState(new IcbmFlightPlan(old.missileId(),old.ownerPlayerId(),old.launchPosition(),old.burnoutPosition(),p.separationPosition(),p.resolvedTarget(),old.launchGameTime(),old.ignitionTicks(),old.boostTicks(),p.revisedCoastTicks(),old.visualSeed(),old.payloadType()));}
