 ...
 *
 * <p>Only the current frontier plus look-ahead is built. Surface data is shared
 * through {@link TerrainSurfaceCache}; no impact-to-sample raycasts are done.</p>
 */
public final class TerrainShockfrontField {
	...
	/** Compatibility entry point. Prefer {@link #buildToDistance}. */
	...
	public List<TerrainShockfrontNode> readyNodes(final double pressureRadius, final int desiredSpokes,
	...
	public List<TerrainShockfrontNode> activeDustNodes(final double pressureRadius, final int desiredSpokes,
		final int maximumNodes, final long gameTime) {
		if (!Double.isFinite(pressureRadius) || pressureRadius <= 0.0 || maximumNodes <= 0) return List.of();
		int count = Math.max(1, Math.min(this.spokes.size(), desiredSpokes));
		int perSpokeLimit = Math.max(1, Math.min(16, (maximumNodes + count - 1) / count + 2));
		List<List<TerrainShockfrontNode>> perSpoke = new ArrayList<>(count);
		for (int index = 0; index < count; index++) {
			TerrainShockfrontSpoke spoke = this.spokes.get(index * this.spokes.size() / count);
			spoke.updateReached(pressureRadius, gameTime);
			perSpoke.add(spoke.activeDustNodesNearFrontier(pressureRadius, perSpokeLimit, gameTime));
		}
		return interleave(perSpoke, maximumNodes);
	}
	...
	private void buildOneSample(final ClientLevel level, final TerrainShockfrontSpoke spoke, final int sampleIndex) {
		...
		TerrainSurfaceCache.SurfaceSample surface = TerrainSurfaceCache.INSTANCE.sample(level, x, z);
		if (surface == null) {
			spoke.markComplete();
			return;
		}
		...
		spoke.addNode(new TerrainShockfrontNode(
			...
			cumulativeDistance,
			this.impactPosition.distanceTo(surface.position()),
			true,
			surface.tintColor()
		));
	}

	private static List<TerrainShockfrontNode> interleave(final List<List<TerrainShockfrontNode>> perSpoke,
		final int maximumNodes) {
		if (perSpoke.isEmpty() || maximumNodes <= 0) return List.of();
		int maximumDepth = 0;
		for (List<TerrainShockfrontNode> nodes : perSpoke) maximumDepth = Math.max(maximumDepth, nodes.size());
		List<TerrainShockfrontNode> selected = new ArrayList<>(Math.min(maximumNodes, perSpoke.size() * maximumDepth));
		for (int layer = 0; layer < maximumDepth && selected.size() < maximumNodes; layer++) {
