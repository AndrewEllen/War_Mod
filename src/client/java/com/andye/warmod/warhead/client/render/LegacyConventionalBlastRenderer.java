...
public final class LegacyConventionalBlastRenderer {
	...
	public static void renderSurfaceFront(final PoseStack.Pose pose, final VertexConsumer buffer,
		...
		int base = switch (lod) { case NEAR -> 640; case MEDIUM -> 320; case FAR -> 120; };
		int samples = Math.min(1_500, Math.max(64, Math.round(base * (0.72F + (float) Math.pow(scale, 1.30)))));
		...
		for (int index = 0; index < samples; index++) {
			...
			float radius = (0.10F + unit(value, 3) * 0.25F) * (0.88F + scale * 0.22F);
			float alpha = fade * (0.18F + 0.38F * (1.0F - unit(value, 4)));
			int tone = 176 + Math.floorMod((int) value, 42);
			billboard(pose, buffer, Mth.cos(angle) * radial, height, Mth.sin(angle) * radial,
				radius, angle, tone, Math.min(224, tone + 4), Math.min(230, tone + 9), alpha,
				0xB000B0, basis, 0.0F, 1.0F, 0.0F, 1.0F);
		}
	}
	...
	public static void renderNuclearReturnFront(final PoseStack.Pose pose, final VertexConsumer buffer,
		...
		int base = switch (lod) { case NEAR -> 480; case MEDIUM -> 240; case FAR -> 90; };
		int samples = Math.min(1_200, Math.max(48, Math.round(base * (0.70F + (float) Math.sqrt(scale)))));
		Basis basis = Basis.from(camera);
		for (int index = 0; index < samples; index++) {
			long value = mix(seed ^ RETURN_SEED ^ (long) index * 0x9E3779B97F4A7C15L);
			float angle = (index + unit(value, 0)) / samples * Mth.TWO_PI;
			float radial = (float) returnRadius + signed(value, 1) * (0.8F + 1.6F * scale);
			float height = 0.10F + unit(value, 2) * (0.55F + 0.50F * scale);
			float radius = (0.10F + unit(value, 3) * 0.22F) * (0.92F + 0.12F * scale);
	...
	private static void renderFire(final PoseStack.Pose pose, final VertexConsumer buffer,
		...
		float scale = Mth.clamp(visualScale, 0.28F, 1.75F);
		int base = switch (lod) { case NEAR -> 1_080; case MEDIUM -> 560; case FAR -> 190; };
		...
		for (int index = 0; index < samples; index++) {
			...
			double upwardVelocity = (0.30 + unit(value, 7) * 0.38) * (0.68 + 0.32 * scale);
			double radial = sourceRadius + localAge * outwardVelocity * (1.0 - 0.58 * smooth(progress));
			...
			float drawRadius = (float) ((0.18 + unit(value, 8) * 0.48) * (0.76 + scale * 0.34)
				* (0.78 + progress * 0.46));
			...
			int red = layer == FireLayer.COOL ? Mth.lerpInt((float) progress, 244, 105) : 255;
			...
			int frame = Math.floorMod((int) Math.floor(localAge / 2.1 + unit(value, 9) * FireballAtlas.FRAME_COUNT),
			...
			billboard(pose, buffer, Mth.cos(angle) * (float) radial, (float) y, Mth.sin(angle) * (float) radial,
				drawRadius, angle + (float) localAge * 0.024F, red, green, blue, alpha, 0xF000F0,
				basis, u0, u1, v0, 1.0F - v0);
		}
	}
	...
	private static void renderSmoke(final PoseStack.Pose pose, final VertexConsumer buffer,
		...
		float scale = Mth.clamp(visualScale, 0.28F, 1.75F);
		int base = switch (lod) { case NEAR -> 1_420; case MEDIUM -> 720; case FAR -> 240; };
		...
		for (int index = 0; index < samples; index++) {
			...
			double radial = Math.sqrt(unit(value, 4)) * (0.9 + 5.0 * scale) * (core ? 0.48 : 1.0)
				+ localAge * (0.014 + unit(value, 5) * 0.040) * (0.70 + 0.42 * scale);
			double rise = localAge * (0.045 + unit(value, 6) * 0.080) * (0.68 + 0.40 * scale);
			double y = craterBase + 1.5 + rise;
			if (age < 48.0) y = Math.min(HE_FIRE_TOP + (age - 5.0) * 0.035, y);
			float drawRadius = (float) ((0.17 + unit(value, 7) * 0.54) * (0.78 + scale * 0.34)
				* (0.72 + progress * 0.76));
			...
			int tone = Mth.clamp((corePass ? 32 : 48) + (int) (unit(value, 8) * (corePass ? 28.0 : 58.0))
				+ (int) (progress * 42.0), 28, 154);
			billboard(pose, buffer, Mth.cos(angle) * (float) radial, (float) y, Mth.sin(angle) * (float) radial,
				drawRadius, angle + (float) localAge * 0.007F, tone, Math.min(158, tone + 4),
				Math.min(168, tone + 10), alpha, corePass ? 0x900090 : 0xA000A0,
				basis, 0.0F, 1.0F, 0.0F, 1.0F);
		}
	}

	private enum FireLayer { CORE, HOT, COOL }
	...
	private static void billboard(final PoseStack.Pose pose, final VertexConsumer buffer,
		...
		vertex(pose, buffer, centerX, centerY, centerZ, -radius, -radius, cosine, sine, u0, v1, red, green, blue, alpha, light, basis);
		vertex(pose, buffer, centerX, centerY, centerZ, -radius, radius, cosine, sine, u0, v0, red, green, blue, alpha, light, basis);
		vertex(pose, buffer, centerX, centerY, centerZ, radius, radius, cosine, sine, u1, v0, red, green, blue, alpha, light, basis);
		vertex(pose, buffer, centerX, centerY, centerZ, radius, -radius, cosine, sine, u1, v1, red, green, blue, alpha, light, basis);
	}
