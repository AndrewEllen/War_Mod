...
public final class PressureWaveSphereRenderer {
	...
	public static void render(final PoseStack.Pose pose, final VertexConsumer buffer,
		...
		render(pose, buffer, physicalRadius, ageTicks, thicknessScale, alphaScale, 1.0F, lod);
	}
	...
	public static void render(final PoseStack.Pose pose, final VertexConsumer buffer,
		...
		renderShell(pose, buffer, radius + thickness * 0.18F, longitudes, latitudes,
			230, 244, 255, (float) alpha, 0.0F, 1.0F);
		renderShell(pose, buffer, Math.max(0.0F, radius - thickness * 1.6F), longitudes, latitudes,
			214, 234, 250, (float) (alpha * 0.56), 0.08F, 0.90F);
		renderShell(pose, buffer, Math.max(0.0F, radius - thickness * 3.2F), longitudes, latitudes,
	...
	private static void renderShell(final PoseStack.Pose pose, final VertexConsumer buffer,
		...
		for (int latitude = 0; latitude < latitudes; latitude++) {
			...
			for (int longitude = 0; longitude < longitudes; longitude++) {
				...
				vertex(pose, buffer, radius, theta0, phi0, red, green, blue, alphaByte,
					(float) longitude / longitudes, vOffset + vScale * latitude / latitudes);
				vertex(pose, buffer, radius, theta1, phi0, red, green, blue, alphaByte,
					(float) longitude / longitudes, vOffset + vScale * (latitude + 1) / latitudes);
				vertex(pose, buffer, radius, theta1, phi1, red, green, blue, alphaByte,
					(float) (longitude + 1) / longitudes, vOffset + vScale * (latitude + 1) / latitudes);
				vertex(pose, buffer, radius, theta0, phi1, red, green, blue, alphaByte,
					(float) (longitude + 1) / longitudes, vOffset + vScale * latitude / latitudes);
			}
		}
	}

	private static void vertex(final PoseStack.Pose pose, final VertexConsumer buffer,
		final float radius, final float theta, final float phi, final int red,
		final int green, final int blue, final int alpha, final float u, final float v) {
