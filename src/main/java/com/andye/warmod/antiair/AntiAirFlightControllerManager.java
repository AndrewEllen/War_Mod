...
public final class AntiAirFlightControllerManager {
    ...
    public static synchronized List<AntiAirFallbackSnapshot> fallbackSnapshots(ServerLevel level) { var map = ACTIVE.get(level); if (map == null) return List.of(); return map.values().stream().filter(c -> c.phase() == AntiAirFlightPhase.FALLBACK).map(c -> new AntiAirFallbackSnapshot(c.plan().interceptorId(), c.plan().ownerPlayerId(), c.currentPosition(), c.currentVelocity(), c.fallbackStart(), c.plan().variant(), !c.completed())).toList(); }
    public static synchronized List<AntiAirPointDefenceSnapshot> pointDefenceSnapshots(ServerLevel level) { var map = ACTIVE.get(level); if (map == null) return List.of(); return map.values().stream().filter(c -> !c.completed()).map(c -> new AntiAirPointDefenceSnapshot(c.plan().interceptorId(), c.plan().ownerPlayerId(), c.plan().affiliation(), c.currentPosition(), c.currentVelocity(), c.phase(), c.plan().variant(), true, c.plan(), c.lockedRoute(), c.routeLockGameTime(), level.getGameTime())).toList(); }
    public static synchronized Optional<StrategicMissileTargetState> targetState(ServerLevel level, UUID interceptorId) { var map = ACTIVE.get(level); var controller = map == null ? null : map.get(interceptorId); return controller == null || controller.completed() ? Optional.empty() : Optional.of(new StrategicMissileTargetState(interceptorId, com.andye.warmod.warhead.WarheadPayloadType.CONVENTIONAL, controller.currentPosition(), controller.currentVelocity(), false, true)); }
    public static synchronized boolean cancelAsInterceptorTarget(ServerLevel level, UUID targetId, UUID interceptorId, Vec3 hit) { var map = ACTIVE.get(level); var controller = map == null ? null : map.get(targetId); return controller != null && !targetId.equals(interceptorId) && controller.cancelAsInterceptorTarget(level); }
