...
public final class WarheadVisualMath {
	...
	public static double reentryWidthProgress(final double progress){return smoothstep(.52,.90,clamp(progress,0,1));}
	public static double reentryElongationProgress(final double progress){return smoothstep(.68,.98,clamp(progress,0,1));}	public static double reentryShimmer(final double elapsedTicks,final long visualSeed){if(!Double.isFinite(elapsedTicks))return 1;double phase=(visualSeed&0xffffL)/65536.0*Math.PI*2;return clamp(1+.075*Math.sin(elapsedTicks*.43+phase)+.045*Math.sin(elapsedTicks*.79-phase*1.7)+.025*Math.sin(elapsedTicks*1.31+phase*.4),.82,1.18);}
