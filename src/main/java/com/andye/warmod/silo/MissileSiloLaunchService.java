...
public final class MissileSiloLaunchService {
    ...
    public static synchronized MissileSiloLaunchResult requestLaunch(ServerLevel level, MissileSiloBlockEntity silo,
        MissileSiloLaunchTrigger trigger, @Nullable UUID player, @Nullable String name, @Nullable TargetCoordinates override) {
        if (silo.pendingLaunchRequestId() != null || EnumSet.of(MissileSiloState.LAUNCHING, MissileSiloState.COOLDOWN,
            MissileSiloState.RELOADING).contains(silo.siloState())) return fail(MissileSiloLaunchFailure.BUSY, "Silo is busy");
        ...
        if (type == null) return fail(MissileSiloLaunchFailure.NO_AMMUNITION, "Silo has no supported ammunition");
    ...
    private static MissileSiloLaunchResult interceptor(ServerLevel level, MissileSiloBlockEntity silo,
        MissileSiloLaunchTrigger trigger, @Nullable UUID player, @Nullable String name, SiloMissileType type) {
        ...
        requests.put(id, new MissileSiloLaunchRequest(id, silo.siloId(), silo.getBlockPos(), trigger, player,
            name == null ? "SERVER" : name, type, target, null, null, null, level.getGameTime(), tickets));
        return MissileSiloLaunchResult.accepted(id);
    }
    public static synchronized void tick(ServerLevel level) {
        var requests = PENDING.get(level); if (requests == null) return;
        ...
        while (iterator.hasNext()) {
            ...
            if (level.getGameTime() - request.creationGameTime() >= MissileSiloConstants.TARGET_PREPARATION_TIMEOUT_TICKS) {
    ...
    public static synchronized void cancel(ServerLevel level, UUID siloId, String reason) { var pending = PENDING.get(level);
        if (pending == null) return; var iterator = pending.values().iterator(); while (iterator.hasNext()) { var request = iterator.next();
            if (!request.siloId().equals(siloId)) continue; release(level, request);
            if (level.getBlockEntity(request.siloCentre()) instanceof MissileSiloBlockEntity silo) silo.fail(reason); iterator.remove(); }
        if (pending.isEmpty()) PENDING.remove(level); }
    public static synchronized void stop() { for (var entry : PENDING.entrySet()) for (var request : entry.getValue().values()) {
        release(entry.getKey(), request); if (entry.getKey().getBlockEntity(request.siloCentre()) instanceof MissileSiloBlockEntity silo) silo.restoreReserved(); }
        PENDING.clear(); }
    private static Vec3 origin(MissileSiloBlockEntity silo) { return new Vec3(silo.getBlockPos().getX() + .5,
        silo.getBlockPos().getY() + 1.4, silo.getBlockPos().getZ() + .5); }
    private static MissileSiloCollisionContext collision(MissileSiloBlockEntity silo) { Set<net.minecraft.core.BlockPos> ignored =
        new LinkedHashSet<>(MissileSiloStructure.positions(silo.getBlockPos(), silo.facing())); ignored.addAll(
        MissileSiloGuidanceFrameStructure.positions(silo.getBlockPos(), silo.facing())); return new MissileSiloCollisionContext(
        silo.siloId(), silo.getBlockPos(), ignored, MissileSiloConstants.MISSILE_COLLISION_WIDTH, MissileSiloConstants.MISSILE_COLLISION_HEIGHT); }
    private static boolean allLoaded(ServerLevel level, Set<ChunkPos> positions) { for (var position : positions)
        if (level.getChunkSource().getChunkNow(position.x(), position.z()) == null) return false; return true; }
    private static void release(ServerLevel level, MissileSiloLaunchRequest request) { IcbmChunkTicketRegistry.releaseAll(level, request.temporaryTickets()); }
    private static MissileSiloLaunchResult fail(MissileSiloLaunchFailure failure, String message) { return MissileSiloLaunchResult.failed(failure, message); }
    private static MissileSiloLaunchResult failInterceptor(MissileSiloBlockEntity silo, MissileSiloLaunchFailure failure, String message) {
        silo.fail(message); return fail(failure, message); }
    private static void notify(ServerLevel level, @Nullable UUID id, String message) { if (id == null) return;
        ServerPlayer player = level.getServer().getPlayerList().getPlayer(id); if (player != null) player.sendSystemMessage(Component.literal(message)); }
}
